...
const PRODUCT_FIELDS: { field: { en: string; fr: string }; required: boolean; note: { en: string; fr: string } }[] = [
  ...
  { field: { en: "Bulk price tiers", fr: "Paliers de prix dégressifs" }, required: false, note: { en: "Rows like \"10 units = 50,000\" — pooled across every picture/size of that product at checkout.", fr: "Lignes du type « 10 unités = 50 000 » — regroupées sur toutes les images/tailles du produit au paiement." } },
  { field: { en: "Images / video", fr: "Images / vidéo" }, required: true, note: { en: "Each picture can become its own orderable line for the customer.", fr: "Chaque image peut devenir sa propre ligne commandable pour le client." } },
...
const SECTIONS: Record<"en" | "fr", PlaybookSection[]> = {
  en: [
    ...
    },
    {
      ...
      body: [
        "One long form, six sections. Only a name and at least one picture are strictly required — everything else has a sensible default so a listing can go live fast and be enriched later.",
      ],
    ...
    },
    {
      ...
      body: [
        ...
        "The order detail page has everything: the item list (picture, colour, size, which numbered image the customer picked, quantity, line price), payment info (method, USSD code used, account name, amount), the customer's shipping info, a timeline of who did what and when, and the messaging panel used for packaging confirmations.",
        'Click the payment screenshot to zoom it full-screen. From here — or from the "Reviewing" tab in the list — Validate moves the order forward and shows a popup confirming it went through, or Cancel/Reject opens a small form requiring a reason, which the customer then sees on their own tracking page.',
      ],
    ...
    },
    {
      ...
      body: [
        "The physical fulfilment side — pulling a validated order off the shelf, boxing it, and telling the customer it's on its way. Organised as three personal queues: Waiting (every Validated order, for anyone with permission), Packing Now (claimed by you via \"Start Packaging\" — which jumps you straight into this tab), and Completed (marked complete by you). Each queue loads a batch of orders at a time and quietly fetches more as you scroll toward the bottom, so a busy day never means loading hundreds of cards up front.",
        ...
        'Tapping "Finish Packing" doesn\'t complete the order outright — a popup asks you to confirm the parcel is fully packed and sealed, shows a loading state while it saves, and only then moves the order into Completed. It\'s the one action here that can\'t be undone by a stray tap.',
        'The "Delivery Team" button opens a roster of name + phone number, editable at any time — this list isn\'t tied to any one order, and ships pre-seeded with the current delivery team so there\'s always someone to call.',
        'Once an order is Packaged, the message box on its detail page turns into the packaging-confirmation composer: attach a picture of the sealed parcel, write a short note, and send. The backend automatically appends a snapshot of the current delivery-team roster to that exact message — presented to the customer as a card with tap-to-call contacts and icon-led pickup steps, not a block of text — so even if the roster changes later, this order keeps the names and numbers that were current the moment it shipped. The customer sees this picture, note, and contact card on their tracking page and in the chat thread.',
      ],
      tip: "The Message Customer box only ever sends the packaging confirmation — it's locked until the order is marked Packaged (nothing to tell the customer yet), stays locked for anyone without SEND_PACKAGING_MESSAGE, and locks again once a confirmation has been sent, so a follow-up send can't silently replace the picture and delivery details already shown to the customer. A \"Delete & Resend\" button takes its place instead — delete the sent confirmation first, and the composer reopens to send a replacement.",
    },
    {
      id: "chat",
      title: "Chat",
  ...
  fr: [
    ...
    },
    {
      ...
      body: [
        "Un long formulaire en six sections. Seuls un nom et au moins une image sont strictement requises — tout le reste a une valeur par défaut raisonnable, pour qu'une fiche puisse être publiée rapidement et enrichie ensuite.",
      ],
    ...
    },
    {
      ...
      body: [
        ...
        "La page de détail réunit tout : la liste des articles (image, couleur, taille, numéro de l'image choisie par le client, quantité, prix de la ligne), les infos de paiement (méthode, code USSD utilisé, nom du compte, montant), les informations de livraison du client, une chronologie de qui a fait quoi et quand, et le panneau de messagerie utilisé pour les confirmations d'emballage.",
        "Cliquez sur la capture d'écran de paiement pour l'agrandir. Depuis cet endroit — ou depuis l'onglet « En vérification » de la liste — Valider fait avancer la commande et affiche une fenêtre confirmant que l'opération a réussi, ou Annuler/Rejeter ouvre un petit formulaire demandant un motif, que le client voit ensuite sur sa propre page de suivi.",
      ],
    ...
    },
    {
      ...
      body: [
        "Le volet physique de l'exécution — sortir une commande validée, l'emballer, et prévenir le client qu'elle est en route. Organisé en trois files personnelles : En attente (toute commande Validée, pour quiconque a la permission), En cours d'emballage (prise en charge par vous via « Démarrer l'emballage », qui vous amène directement sur cet onglet), et Terminé (marquée comme complète par vous). Chaque file charge un lot de commandes à la fois et en récupère discrètement d'autres à mesure que vous faites défiler vers le bas, pour qu'une journée chargée ne signifie jamais charger des centaines de cartes d'un coup.",
        ...
        "Appuyer sur « Terminer l'emballage » ne clôture pas la commande directement — une fenêtre vous demande de confirmer que le colis est bien emballé et scellé, affiche un état de chargement pendant l'enregistrement, puis fait passer la commande dans Terminé. C'est la seule action ici qu'un geste malencontreux ne peut pas déclencher par erreur.",
        "Le bouton « Équipe de livraison » ouvre une liste de noms et numéros de téléphone, modifiable à tout moment — cette liste n'est liée à aucune commande en particulier, et est préremplie dès le départ avec l'équipe de livraison actuelle pour qu'il y ait toujours quelqu'un à appeler.",
        "Une fois une commande Emballée, la zone de message de sa page de détail devient le compositeur de confirmation d'emballage : joignez une image du colis scellé, rédigez une note courte, et envoyez. Le serveur ajoute automatiquement un instantané de l'équipe de livraison actuelle à ce message précis — présenté au client sous forme de carte avec des contacts à appeler d'un geste et des étapes de retrait illustrées par des icônes, pas un bloc de texte — afin que même si l'équipe change plus tard, cette commande garde les noms et numéros en vigueur au moment de l'envoi. Le client voit cette image, cette note et cette carte de contact sur sa page de suivi et dans le fil de discussion.",
      ],
      tip: "La zone Message au client n'envoie que la confirmation d'emballage — elle est verrouillée jusqu'à ce que la commande soit marquée Emballée (rien à dire au client avant), reste verrouillée pour quiconque n'a pas SEND_PACKAGING_MESSAGE, et se reverrouille une fois une confirmation envoyée, afin qu'un envoi ultérieur ne puisse pas remplacer silencieusement l'image et les informations de livraison déjà montrées au client. Un bouton « Supprimer et renvoyer » prend sa place à la place — supprimez la confirmation envoyée, et le compositeur se rouvre pour en envoyer une nouvelle.",
    },
    {
      id: "chat",
      title: "Chat",
