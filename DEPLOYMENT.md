...
| File | Location | Notes |
|---|---|---|
| Frontend `.env` | `~/riskyc/riskyc-frontend/.env` on server | `API_URL` (build-time, needs `full-restart`), `FRONTEND_PORT`, `SITE_URL` (runtime, plain `restart` ok but sitemap cache needs `full-restart`) |
| Backend `.env` | `~/riskyc/riskyc-backend/.env` on server | `FRONTEND_URL`, `MINIO_PUBLIC_URL`, `BACKEND_PORT`, `MINIO_API_PORT`, `MINIO_CONSOLE_PORT`, `POSTGRES_PASSWORD`, `MINIO_ROOT_USER`/`PASSWORD`, `JWT_SECRET` |
