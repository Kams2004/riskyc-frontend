export const adminOrders = {
  status: {
    pending: "Pending",
    awaitingPayment: "Awaiting Payment",
    reviewing: "Under Review",
    validated: "Validated",
    packaging: "Packaging",
    packaged: "Packaged",
    cancelled: "Cancelled",
  },
  payment: {
    orangeMoney: "🟠 Orange",
    momo: "🟡 MoMo",
    orangeMoneyFull: "🟠 Orange Money",
    momoFull: "🟡 MTN Mobile Money",
  },
  counts: {
    productOne: "{{count}} product",
    productOther: "{{count}} products",
    unitOne: "{{count}} unit",
    unitOther: "{{count}} units",
  },
  confirm: {
    rejectTitle: "Reject order?",
    rejectMessage:
      "This will cancel the order and notify the customer (in-app and by push, if they've enabled it) with the reason below.",
    rejectConfirmLabel: "Reject",
    rejectReasonLabel: "Reason for rejection",
    rejectReasonPlaceholder: "e.g. Payment screenshot doesn't match the order total",
    deleteConfirmationTitle: "Delete packaging confirmation?",
    deleteConfirmationMessage: "This removes the sent picture and delivery details so you can send a replacement — the customer will no longer see the old one. The new one will still include the current delivery team roster.",
  },
  list: {
    title: "Orders",
    totalOrdersOne: "{{count}} total order",
    totalOrdersOther: "{{count}} total orders",
    searchPlaceholder: "Search order ID…",
    scanButton: "Scan",
    tableView: "Table",
    gridView: "Grid",
    filterAll: "All",
    table: {
      orderId: "Order ID",
      items: "Items",
      method: "Method",
      total: "Total",
      status: "Status",
      actions: "Actions",
      view: "View",
    },
    empty: "No orders found",
    noPaymentYet: "No payment yet",
    validate: "Validate",
    cancel: "Cancel",
    view: "View",
    showingRange: "Showing {{from}}–{{to}} of {{total}} orders",
    prev: "Prev",
    next: "Next",
  },
  detail: {
    back: "Back",
    loading: "Loading…",
    notFound: "Order not found.",
    backToOrders: "← Back to orders",
    placedOn: "Placed on {{date}}",
    validateOrder: "Validate Order",
    cancelOrder: "Cancel Order",
    cancel: "Cancel",
    progressHeading: "Order Progress",
    rejectionReasonHeading: "Rejection Reason",
    orderItemsHeading: "Order Items",
    photoLabel: "Picture {{index}}",
    each: "each",
    total: "Total",
    paymentInfoHeading: "Payment Info",
    method: "Method",
    codeUsed: "Code Used",
    account: "Account",
    amount: "Amount",
    noPaymentMethod: "No payment method selected yet.",
    shippingInfoHeading: "Shipping Info",
    recipient: "Recipient",
    phone: "Phone",
    deliveryType: "Delivery type",
    deliveryTypeDelivery: "Delivery",
    deliveryTypePickup: "Pickup",
    town: "Town",
    street: "Street / Address",
    paymentProofHeading: "Payment Proof",
    paymentProofAlt: "Payment proof",
    paymentProofFullscreenAlt: "Payment proof fullscreen",
    submittedByCustomer: "Submitted by customer",
    approveValidate: "Approve & Validate",
    reject: "Reject",
    timelineHeading: "Timeline",
    created: "Created",
    lastUpdate: "Last Update",
    status: "Status",
    changedBy: "Changed by",
    packagingStarted: "Packaging started",
    packagingDone: "Packaging done",
    changeStatus: "Change Status",
    messageCustomerHeading: "Message Customer",
    messageHintWithChat:
      "Sends to this customer's live chat — same thread as the storefront chat widget.",
    messageHintGuest:
      "This was a guest checkout with no account — it still sends to the order's tracking page and chat widget on the same device.",
    messageHintPackaging:
      "Confirm the package is ready — the delivery team's contact info is attached automatically.",
    messageHintPackagingNoPermission:
      "You don't have permission to send the packaging confirmation — ask whoever validated or packaged this order, or a Super Admin.",
    messageHintPackagingInProgress:
      "You'll be able to send the packaging confirmation once this order is marked packaged.",
    messageHintAlreadySent:
      "A packaging confirmation was already sent — delete it below to send a replacement.",
    messagePlaceholder: "Type a message to the customer…",
    messagePackagingPlaceholder: "Optional note to include with the packaging confirmation…",
    messageSent: "✓ Message Sent!",
    messageSendError: "Couldn't send — please try again.",
    photoNotReadyError: "That picture didn't come through — please try again.",
    sendMessage: "Send Message",
    sendPackagingConfirmation: "Send Packaging Confirmation",
    packagingConfirmationSent: "Packaging confirmation sent with delivery details",
    deleteConfirmationAndResend: "Delete & Resend",
    attachPhoto: "Attach a picture",
  },
  steps: {
    orderPlaced: "Order Placed",
    payment: "Payment",
    review: "Review",
    validated: "Validated",
    packaging: "Packaging",
    packaged: "Packaged",
  },
  treatment: {
    title: "Packing",
    subtitle: "Validated orders move here to be packaged before shipping/pickup.",
    tabWaiting: "Waiting for Packaging",
    tabInProgress: "Packaging Now",
    tabDone: "Completed",
    empty: "Nothing here right now",
    startPackaging: "Start Packaging",
    starting: "Starting…",
    markDone: "Finish Packing",
    saving: "Finishing…",
    startedLabel: "started",
    finishedLabel: "finished",
    noPermission: "You don't have permission to manage orders",
    notYourPackage: "Only the admin who started packing this order (or a super admin) can mark it done",
    errorStart: "Failed to start packaging.",
    errorComplete: "Failed to mark packaging done.",
    deliveryTeamButton: "Delivery Team",
    alreadyPacked: "Already Being Packed",
  },
  finishPacking: {
    confirmTitle: "Finish packing?",
    confirmBody:
      "Confirm that this order has been fully packed and sealed. Next you'll take a picture of the sealed parcel to send the customer, along with the delivery team's contact details.",
    cancel: "Cancel",
    confirmButton: "Yes, Finished",
    captureTitle: "Snap the Sealed Parcel",
    captureBody: "Take a picture of the sealed parcel to confirm packaging is done.",
    photoAlt: "Sealed parcel",
    previewCaption: "Delivery details are already attached — click send to confirm and notify the customer.",
    openCamera: "Open Camera",
    retake: "Retake",
    sending: "Sending…",
    sendButton: "Send & Confirm",
    photoNotReadyError: "That picture wasn't ready yet — please try again.",
    genericError: "Something went wrong — please try again.",
  },
  deliveryContacts: {
    title: "Delivery Team",
    hint: "This list is attached automatically to every packaging confirmation sent to a customer.",
    loading: "Loading…",
    empty: "No delivery contacts yet.",
    namePlaceholder: "Name",
    phonePlaceholder: "Phone number",
    save: "Save",
    cancel: "Cancel",
    addButton: "Add delivery contact",
    errorLoad: "Failed to load delivery contacts.",
    errorSave: "Failed to save contact.",
    errorDelete: "Failed to delete contact.",
  },
  scan: {
    title: "Scan Order Code",
    startingCamera: "Starting camera…",
    instructions: "Point the camera at the QR code on the customer's receipt.",
    notAnOrderCode: "This isn't a Riskyc Fashion order code.",
    cameraError: "Couldn't access the camera — check permissions and try again.",
  },
};
